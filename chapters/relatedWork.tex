...
Dank dieser Modifizierungen eignet sich das Distanzmaß gut für den Vergleich von Trajektorien:
...
% Spurerkennung (realistische Geometrien) meist nur auf Basis statistischer Modelle
% "reale" Spurerkennung (wie in Hsieh et al. nur für gerade Spuren)
...
% Erkennung von Fahrspuren unterschiedlicher Spurdichten ????
...
kleine Auswahl dar. Da die in den Arbeiten publizierten Verfahren teils sehr unterschiedlich sind und
zur Erreichung verschiedenster Ziele eingesetzt werden, muss in dieser Arbeit ein Verfahren identifiziert
werden, welches sich möglichst gut für die Analyse von Fahrzeugtrajektorien eignet.
Ein Defizit vieler vorhandener Arbeiten ist, dass diese meist mit wenigen unterschiedlichen
Trajektorie-Datensätzen arbeiten und die verwendeten Daten selten Defekte wie Unterbrechungen, Ausreißer
et cetera aufweisen. In einigen Arbeit wird teilweise sogar lediglich mit generierten Trajektoriedaten
gearbeitet. In der vorliegenden Thesis muss daher zudem ein zuverlässiges Verfahren zur Bereinigung
von Trajektoriedaten entwickelt werden.
...
in den allermeisten Fällen nicht den realen Verläufen der Fahrbahnen. Üblicherweise werden Fahrspuren
anhand statistischer Verfahren, wie in \cite{WeimingHu2006} oder in \cite{Teng2015} beschrieben, ermittelt.
\cite{Hsieh2006} ermitteln in ihrer Arbeit reale Spur-Begrenzungslinien, allerdings nur für kurze, parallele
Fahrspuren eines Autobahnabschnittes. In dieser Arbeit sollen hingegen reale Spurgeometrien für
unterschiedliche Straßenabschnitte bestimmt werden, weshalb das hierzu eingesetzte Verfahren selbst entwickelt
werden muss.

Zudem muss ein Ansatz zur Partitionierung von Fahrspuren entwickelt werden, da sich erkannte Fahrspuren
