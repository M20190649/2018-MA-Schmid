...
eine Hülle definiert. Im diesem Abschnitt wird beschrieben, wie die Hüllen erstellt werden.
...
Diesen Ansatz verwenden auch Hsieh et al. in ihrer Arbeit. Sie gehen jedoch, da die von ihnen verwendeten
Aufnahmen von einer statischen Kamera über einer Autobahn stammen, davon aus, dass alle Fahrspuren parallel
zueinander verlaufen. Da das in dieser Arbeit entwickelte Spurerkennungsmodul mit unterschiedlichen
...
jene zu finden, welche parallel zueinander verlaufen und auf Basis dieser die Spurhüllen zu definieren.
...
%TODO: Quelle RAA, RAL
Der Wert für $\sigma$ wurde auf Basis der in Deutschand geltenden \textit{``Richtlinien für die Anlage von Autobahnen''} (RAA)
und der \textit{``Richtlinien für die Anlage von Landstraßen''} (RAL) bestimmt. Diese Regelwerke spezifizieren
die in der Bundesrepublik derzeit zulässigen Straßenquerschnitte. Die in ihnen festgelegten
Spurbreiten bewegen sich zwischen 3.25 und 3.75 Metern.
Um auch besonders breite benachbarte Spur-Paare identifizieren zu können, wurde für $\sigma$ der Wert 4m gewählt.

Die auf diese Weise gefundenen Trajektorie-Paare sind potenzielle parallele Spuren.
Der Algorithmus welcher prüft, ob zwei Spuren tatsächlich parallel sind, ist in Listing \ref{lst:pseudo_checkParallel} beschrieben. 
Die zu vergleichenden Mittellinien werden jeweils in eine Reihe von Richtungsvektoren
umgewandelt. Anschließend werden die paarweisen Differenzen zwischen den Vektoren berechnet und gemittelt.
...
Als Wert für $\delta$ wurde experimentell 0.1 bestimmt. In den verschiedenen Test-Datensätzen konnten
auf die oben beschrieben Weise zuverlässig parallele Spur-Paare identifiziert werden. Anhand dieser
werden im nächsten Schritt die Spurhüllen bestimmt.
...
deren mittleren Abstand zueinander. Verlaufen zu einer Spurlinie zwei Linien parallel, werden
die Abstände zu beiden berechnet und der kleinere als Spurbreite gewählt.
Aus allen auf diese Weise bestimmten Spurbreiten wird die kleinste ausgewählt und jenen Spuren zugewiesen,
welche noch keine Breite besitzen.

charakteristischer 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{../resources/img/umsetzung/U2/calc_env_point}
    \caption{Bestimmung Spürhüllen}
    \label{fig:real2_envelope_definition_concept}
\end{figure}
...
$$\overrightarrow{AC} \cdot \overrightarrow{X} = 0$$

\begin{ceqn}
\begin{align}
    h: \vec{x} = \overrightarrow{OB} + t \cdot \overrightarrow{X}
\end{align}
\end{ceqn}

% Evtl. am ende schonmal beispiel screenshot spuren in Trackerapplication ohne partitionierung
