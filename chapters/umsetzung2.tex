...
Geometrie-Informationen der Fahrspuren abgeleitet werden. Eine Spur-Geometrie besteht grundlegend
...
Die gesuchten Mittellinien verlaufen durch die Mitten der Trajektorie-Cluster.
Die einzelnen Bewegungsbahnen besitzen jeweils leichte Abweichungen von dieser \textit{``Ideallinie''}.
...
Der in dieser Arbeit entwickelte Ansatz zur Bestimmung der Mittellinien macht sich die von Atev et al.
...
Auf Basis der bestimmten Mittellinien werden anschließend die Spurhüllen definiert.
...
Spurmittellinien und nutzt deren relative Lage zueinander. Ansätze hierzu stammen aus den Arbeiten von
...
\begin{figure}[H]
    ...
    \caption[Konzept Bestimmung Spürhüllen]
            {Konzept Bestimmung Spürhüllen - Ermittlung der Spurbreiten anhand des Abstandes paralleler Spuren}
...
Bevor für eine Mittellinie die sie umgebende Spurhülle definiert werden kann, muss die Breite der Spur
ermittelt werden. Diese ergibt sich, für die im vorherigen Schritt identifizierten parallelen Spur-Paare, aus
...
Nachdem die Breiten für alle Fahrspuren bestimmt wurden, können die Hülllinien berechnet werden.
Hierzu werden für jeden Punkt einer Mittellinie zwei zugehörige Hüllpunkte berechnet.
...
\begin{figure}[H]
    ...
    \caption{Konzept Berechnung Spurhüllpunkte}
...
von Fahrspuren zu entfernen, werden in einem weiteren Schritt Pseudo-Spur-Geometrien entfernt.
...
gänzlich oder fast gänzlich überlagert. Diese Art von Pseudo-Spur ist leicht zu identifizieren.
Es wird für jede Spur geprüft, ob eine andere Spur existiert, welche sie zu 95\% oder mehr überlagert.
Wie die Überlagerung von zwei Spuren bestimmt wird, ist in Abschnitt \ref{sec:real2_lane_partitioning}
beschrieben.

Geometrien, welche nicht eine Spur sondern den Wechsel zwischen zwei Spuren darstellen, können
existieren, wenn die Trajektoriedaten viele Spurwechselvorgänge enthalten und in der Clusteranalyse für
diese ein extra Cluster erstellt wird. Um diese Spuren zu identifizieren, müssen Kriterien eines
Spurwechselvorgangs definiert werden. Diese zeichnen sich primär dadurch aus, dass sie auf einer Spur
...
Grundlegend wird zuerst das Krümmungsverhalten der zwei Fahrspuren im Bereich ihres Schnittpunktes untersucht.
...
welche Spur geteilt wird. Der Anwender des \textit{Vehicle-Trackers} kann das Verhalten des Algorithmus an dieser Stelle
...
\begin{figure}[H]
    \centering
    \subfloat[]{{
        \includegraphics[align=c, width=0.45\linewidth]{resources/img/umsetzung/U2/partitionedLanes}
    }}
    \subfloat[]{{
        \includegraphics[align=c, width=0.5\linewidth]{resources/img/umsetzung/U2/result_lanePartitioned_Screenshot}
    }}
    \caption[Ergebnisse Spur-Partitionierung]
            {Plot der partitionierten Spur-Geometrien a), Visualisierung der Ergebnisse in der Anwendung \textit{Vehicle-Tracker} b)}
...
da sie die realen Fahrspurverläufe gut wiederspiegeln und keine Überlagerungen mehr existieren.
...
Diese verursacht eine falsche Schätzung der Spurbreite (siehe Abschnitt \ref{sec:real2_define_lane_envelope}).
...
\begin{figure}[H]
    ...
    \caption[Beispiel zu schmale Spur-Geometrien]
            {Beispiel zu schmaler Spur-Geometrien - Parallele rote und schwarze Spur-Geometrien grenzen nicht aneinander an}
    \label{fig:real2_pre_opt}
\end{figure}

Um Effekte wie diesen zu korrigieren, werden nach der Partitionierung nochmals die Breiten aller Spur-Geometrien
...
Spur-Geometrien können anschließend die Spuren in die Anwendung \textit{Vehicle-Tracker} eingefügt werden.
...
\begin{figure}[H]
    ...
    \caption{Datenmodell der Fahrspurdefinition}
...
Die Fahrspuren und Querstrecken werden im Fall der automatisch erkannten Fahrbahnen anhand der Spurhüllen
...
In Abbildung 6.29 ist das Ergebnis der Spurerstellung dargestellt.
...
Nachdem die Fahrspuren in der \textit{Vehicle-Tracker} Anwendung angelegt wurden, können sie zur Analyse
