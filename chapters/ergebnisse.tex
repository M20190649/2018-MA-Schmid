...
wurde zudem ein entwickeltes Verfahren vorgestellt, mit dessen Hilfe der Parameter $\epsilon_{DBSCAN}$ 
automatisch angepasst werden kann, wenn die Qualität der ermittelten Spur-Cluster nicht den Erwartungen entspricht.
...
keine automatische Anpassung der Parameter vorgenommen wird, weshalb nur über eine manuelle Anpassung
die Spur-Cluster zuverlässig identifiziert werden können. Dies trifft beispielsweise auf den Datensatz
\textit{Steinheim} zu. Die unter Einsatz der Standardparameter ermittelten Spur-Cluster sind in Abbildung
\ref{fig:results_clusters_steinheim} a) dargestellt.
...
nicht automatisch unter Verwendung eines angepassten Parameters wiederholt. Um die zweite, nach hinten verlaufende Fahrspur
zu identifizieren, muss $\epsilon_{DBSCAN}$ manuell angepasst werden. Das Ergebnis ist in Abbildung
\ref{fig:results_clusters_steinheim} b) dargestellt.
...
ununterbrochene Trajektorien zur Beschreibung einer Spur vorliegen.
...
\begin{figure}[H]
    ...
    \subfloat[Steinheim (UPDATEN)]{{
        \includegraphics[align=c, width=0.35\linewidth]{resources/img/results/Steinheim/improvedLanes}
    }}
