...
% Die Qualität des Ansatzes konnte auch
% durch Anwendung auf weitere Datensätze verifiziert werden.
...
\subsection{Automatische Anpassung der Clustering-Parameter}
...
genannten Standardparameter, Spur-Cluster in den meisten Trajektoriedatensätzen zuverlässig.
Das Standardverfahren stößt allerdings an seine Grenzen, wenn sich die Trajektorien unterschiedlicher Fahrspuren
...
\begin{figure}[H]
    \centering
    \subfloat[]{{
        \includegraphics[align=c, width=0.31\linewidth]{resources/img/umsetzung/U1/Plots_Duesseldorf/preProTraj}
    }}
    \subfloat[]{{
        \includegraphics[align=c, width=0.31\linewidth]{resources/img/umsetzung/U1/Plots_Duesseldorf/filteredClusters_03}
    }}
    \subfloat[]{{
        \includegraphics[align=c, width=0.31\linewidth]{resources/img/umsetzung/U1/Plots_Duesseldorf/filteredClusters}
    }}
...
konnte. Das gewünschte Ergebnis wird allerdings erzielt, wenn der Standardwert für den Parameter $\epsilon_{DBSCAN}$
...
Das Verfahren zur automatischen Anpassung des Clustering-Parameters nutzt die Spur-Geometrien, welche auf Basis
...
und sich nicht überholen. Ein entsprechendes Diagramm ist in Abbildung \ref{fig:real1_way_time_diagramms} a) zu sehen.
Beschreibt eine Spur-Geometrie hingegen, aufgrund einer fehlerhaften Clusteranalyse, mehrere reale Fahrspuren,
dann existieren in dem dafür erstellten Weg-Zeit-Diagramm Überschneidungen von Trajektorien, da die Fahrzeuge
sich auf den realen Fahrspuren überholen können. Ein solches Diagramm ist in Abbildung \ref{fig:real1_way_time_diagramms}
b) dargestellt.
...
\begin{enumerate}
    \item Durchführung Spurerkennung
    \item Erstellung von Weg-Zeit-Trajektorien auf Basis der Spur-Geometrien
    \item Überprüfung auf Überschneidungen in Trajektorien:
    \begin{enumerate}
        \item Anzahl Überschneidungen über Grenzwert $\sigma$: Zurück zu 1) und Anpassung von $\epsilon_{DBSCAN}$
        \item Anzahl Überschneidungen unter Grenzwert $\sigma$: Verwende Spur-Geometrien
    \end{enumerate}
\end{enumerate}
...
Unter Verwendung des Verfahrens werden auch im Fall des \textit{Düsseldorf} Datensatzes, die erwarteten
...
% Beschreibung Problem (siehe Auswertung) --> Bei langen Überlagerungen der Spuren anderer epsilon Parameter notwendig
% Richtige Lösung bei Parameter 0.1
% Zeigen Bilder Plots Düsseldorf (siehe Auswertung)
% --> Notwendigkeit für adaptive Anpasssung von Parameter
% Beschreibung vorgehen:
%   Verfahren nutzt nicht die unmittelbaren Ergebnisse der Clusteranalyse, sondern die Fahrspur-Geometrien, die auf Basis der
%   identifizierten Cluster ermittelt werden (Beschreibung in Abschnitt 6.3)
%   Erstellung von Weg-Zeit-Linien für Trajektorien, welche Bewegung der Trajektorien auf den entsprechenden Fahrbahnen beschreiben
%   Überprüfung ob diese Linien sich kreugen. Kreuzungen können nicht vorkommen, wenn Spurgeometrie tatsächlich nur eine beschreibt
% Zeigen Plots Weg-Zeit-Diagramme --> 2 Beispiele mit und ohne Kreuzungen
% Beschreibung Algo. --> Bestimmung Anzahl Kreuzungen. Prüfen ob Spur existiert, mit mehr als 20% kreuzungen
% --> Wenn ja Parameter anpassen und Spurbestimmung erneut durchlaufen